...
class Personal extends Model
{
    ...
    protected $fillable=['nombre','apellido','ci','celular','cargo','fecha_nac','unidad_id','foto'];
    ...
    public static function setFoto($foto, $actual = false) //foto (al crear), actual (al editar)
    {
        if ($foto) {
            if ($actual) {
                Storage::disk('public')->delete("imagenes/fotos/$actual"); // si es actual borra la anterior
            }
            ...
            Storage::disk('public')->put("imagenes/fotos/$imageName", $imagen->stream()); //guarda la imagen
